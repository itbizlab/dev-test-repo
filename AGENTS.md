...
EIDOC is a Docker Compose-based telemedicine platform. The repository contains infrastructure orchestration files (`docker-compose.yml`, `Dockerfile`) plus minimal development scaffolding. The full PHP application source is expected in `./www/` and Docker service configs in `./docker/`.

## Cursor Cloud specific instructions

### Starting Docker

The Docker daemon must be started manually before running services (Cloud VM uses fuse-overlayfs + iptables-legacy):
...
- The `callme` service will restart continuously unless the actual MiroTalk app files are present in `./www/call-me/app/`. This does not affect the core PHP application.
- Nginx uses deferred DNS resolution for the `callme` upstream so it can start even when `callme` is unhealthy.
- PHP app health check lives at `./www/public/index.php` and verifies PostgreSQL + Redis connectivity.
- PostgreSQL health check gates the PHP container startup (`service_healthy` condition).
- Empty mount directories (`docker/nginx/letsencrypt`, `docker/nginx/www`, `www/call-me/app`, `www/call-me/public`) must exist before `docker compose up`.
...
```bash
curl http://localhost/          # PHP health check (JSON)
